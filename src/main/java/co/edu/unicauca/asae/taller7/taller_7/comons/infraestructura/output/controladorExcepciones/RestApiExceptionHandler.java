...
public class RestApiExceptionHandler {
    ...
    public ResponseEntity<Error> handleReglaNegocioException(final HttpServletRequest req, ReglaNegocioExcepcion ex, Locale locale) {
        Error error = ErrorUtils.crearError(ex.getCodigoError().getCodigo(), ex.formatException(), HttpStatus.BAD_REQUEST.value())
                .setUrl(req.getRequestURL().toString()).setMetodo(req.getMethod());
        return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST);
    }
    
    @ExceptionHandler(EntidadNoExisteException.class)
    public ResponseEntity<Error> handleEntidadNoExisteException(final HttpServletRequest req, EntidadNoExisteException ex, Locale locale) {
        Error error = ErrorUtils.crearError(ex.getCodigo(), ex.getMessage(), HttpStatus.NOT_FOUND.value())
                .setUrl(req.getRequestURL().toString()).setMetodo(req.getMethod());
        return new ResponseEntity<>(error, HttpStatus.NOT_FOUND);
    }
    
    @ExceptionHandler(EntidadYaExisteException.class)
    public ResponseEntity<Error> handleEntidadYaExisteException(final HttpServletRequest req, EntidadYaExisteException ex, Locale locale) {
        Error error = ErrorUtils.crearError(ex.getCodigo(), ex.getMessage(), HttpStatus.CONFLICT.value())
                .setUrl(req.getRequestURL().toString()).setMetodo(req.getMethod());
